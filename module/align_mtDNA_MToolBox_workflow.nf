...
process align_mtDNA_MToolBox {
    ...
    ext log_dir_suffix: {"/${sample_name}"},
        containerOptions: { gmapdb, mt_ref_genome_dir -> "--platform linux/amd64 ${params.container_mount_flag} \"${gmapdb}:/src/gmapdb/\" ${params.container_mount_flag} \"${mt_ref_genome_dir}:/src/genome_fasta/\""}(params.gmapdb, params.mt_ref_genome_dir)
